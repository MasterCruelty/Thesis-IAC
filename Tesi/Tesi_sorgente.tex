...
In primo luogo è stata creata una singola macchina virtuale tramite Azure, scegliendo le specifiche adatte a sostenere i requisiti di sistema richiesti dalle tecnologie di sviluppo già citate. Dunque successivamente vi sono stati installati i seguenti strumenti:
\begin{itemize}
\item \textit{Azure CLI} \\
La command-line di Azure è stata utile a livello di supporto. \\ Nel momento in cui si renceva necessario effettuare delle modifiche minori alla macchina, scrivere comandi direttamente da shell risultava molto più comodo ed efficiente rispetto a fare la stessa modifica attraverso il portale web.\\
...
La command-line di Kubernetes è stata ovviamente fondamentale per gestire le risorse del cluster AKS(Azure Kubernetes Service). 
\end{itemize}
